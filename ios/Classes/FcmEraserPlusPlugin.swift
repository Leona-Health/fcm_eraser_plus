...
public class FcmEraserPlusPlugin: NSObject, FlutterPlugin {
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: ChannelConstant.ERASER_CHANNEL, binaryMessenger: registrar.messenger())
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case MethodsConstant.SET_BADGE:
            ...
            NotificationService.shared.setBadges(count: count ?? 0, isClear: isClear ?? true)
            
            result(true)
        case MethodsConstant.ACTIVE_TAGS:
            Task{
                let activeTags = await NotificationService.shared.getActiveTags()
